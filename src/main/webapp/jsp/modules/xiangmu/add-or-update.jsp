                        ...
                        <form id="addOrUpdateForm">
                            ...
                            <!-- 当前表的字段 -->
                                ...
                                <input id="yonghuId" name="yonghuId" type="hidden">
                                   ...
                                   <div class="form-group col-md-6">
                                       <label>项目进度</label>
                                       <select id="xiangmujinduTypesSelect" name="xiangmujinduTypes" class="form-control">
                                       </select>
                                   </div>
                                   <div class="form-group  col-md-6">
                                       <label>项目详情</label>
                                    ...
                                    <div class="form-group col-md-6">
                                        <label>项目文件</label>
                                        <input name="file" type="file" class="form-control-file" id="xiangmuFileupload">
                                        <label id="xiangmuFileName"></label>
                                        <input name="xiangmuFile" id="xiangmuFile-input" type="hidden">
                                    </div>
...
<script>
    ...
    function upload() {
        ...
        $('#xiangmuFileupload').fileupload({
            url: baseUrl + 'file/upload',
            headers: {token: window.sessionStorage.getItem("token")},
            dataType: 'json',
            done: function (e, data) {
                document.getElementById('xiangmuFile-input').setAttribute('value', baseUrl + 'file/download?fileName=' + data.result.file);
                document.getElementById('xiangmuFileName').innerHTML = "上传成功!";
            }
        });
    ...
    <!-- 初始化当前表的下拉框 -->
        ...
        function initializationXiangmujindutypesSelect(){
            var xiangmujinduTypesSelect = document.getElementById('xiangmujinduTypesSelect');
            if(xiangmujinduTypesSelect != null && xiangmujinduTypesOptions != null  && xiangmujinduTypesOptions.length > 0 ){
                for (var i = 0; i < xiangmujinduTypesOptions.length; i++) {
                    xiangmujinduTypesSelect.add(new Option(xiangmujinduTypesOptions[i].indexName,xiangmujinduTypesOptions[i].codeIndex));
                }
            }
        }
        ...
        function initializationyonghuSelect() {
            ...
            if(yonghuSelect != null && yonghuOptions != null  && yonghuOptions.length > 0 ) {
                for (var i = 0; i < yonghuOptions.length; i++) {
                        yonghuSelect.add(new Option("姓名:"+yonghuOptions[i].name+" 身份证号:"+yonghuOptions[i].idNumber, yonghuOptions[i].id));
                }
    ...
    function setSelectOption() {
        ...
        if(xiangmujinduTypesSelect != null && xiangmujinduTypesOptions != null  && xiangmujinduTypesOptions.length > 0 ) {
            for (var i = 0; i < xiangmujinduTypesOptions.length; i++) {
                if (xiangmujinduTypesOptions[i].codeIndex == ruleForm.xiangmujinduTypes) {//下拉框value对比,如果一致就赋值汉字
                        xiangmujinduTypesSelect.options[i].selected = true;
                }
            }
        }
        <!--  级联表的下拉框回显  -->
    ...
    function validform() {
        return $("#addOrUpdateForm").validate({
            rules: {
                ...
                xiangmujinduTypes: "required",
                xiangmuContent: "required",
                xiangmuFile: "required",
                insertTime: "required",
                updateTime: "required",
            },
            messages: {
                ...
                xiangmujinduTypes: "项目进度不能为空",
                xiangmuContent: "项目详情不能为空",
                xiangmuFile: "项目文件不能为空",
                insertTime: "项目开始时间不能为空",
                updateTime: "项目结束时间不能为空",
            }
        }).form();
    }
    ...
    <!--  当前表的数据回显  -->
        ...
        $("#xiangmuFile").val(ruleForm.xiangmuFile);
    ...
    $(document).ready(function () {
        ...
        // 初始化下拉框
            ...
            initializationXiangmujindutypesSelect();
            <!--  初始化级联表的下拉框  -->
